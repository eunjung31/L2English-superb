...
class GoPDataset(Dataset):
    def __init__(self, set, am='librispeech'):
        ...
        # Load the dataset
        data_path = f'/home/eyeo2/workspace/gopt/data/{dir}'
        if set == 'train':
...
sd = torch.load('/home/eyeo2/workspace/gopt/pretrained_models/gopt_librispeech/best_audio_model.pth', map_location='cpu')
...
train_dataset = GoPDataset(set='train') ## audio1 (ranking)
train_loader = DataLoader(train_dataset, batch_size=2500, shuffle=False)

# Load the test dataset
test_dataset = GoPDataset(set='test') ## audio2 (ranking)
